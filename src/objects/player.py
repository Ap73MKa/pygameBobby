...
from pygame import K_DOWN, K_LEFT, K_RIGHT, K_UP, K_a, K_d, K_s, K_w, Vector2
from pygame.key import get_pressed
from pygame.sprite import Group, Sprite

from src.config import configure
from src.misc import Animation, PathManager, SpriteSheet
...
class Player(Sprite):
    def __init__(self, pos, group: Group, collision_group: Group):
        ...
        # movement
        self.image_offset = Vector2(*self.image.get_size()) - Vector2(
            configure.TITLE_SIZE
        )
    ...
    def input(self):
        ...
        if self.direction:
            ...
            self.target_pos += self.direction * configure.TITLE_SIZE
    ...
    def move(self, delta: float):
        ...
        if distance <= self.move_speed * delta:
            self.pos = self.target_pos.copy()
            self.direction *= 0
            self.step_count += 1
        else:
            speed_factor = min(self.move_speed * delta / distance, 1)
